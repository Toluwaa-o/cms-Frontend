      ...
      {report ? (
        <>
          <div>
            ...
            <p>
              <span>Title:</span> {report.data.report.title}
            </p>
            <p>
              <span>Reported at:</span> {day}
              {day === 1
                ? "st"
                : day === 2
                ? "nd"
                : day === 3
                ? "rd"
                : "th"}, {months[month]} {year}, {timeCode} {period}
            </p>
            <p>
              <span>Category:</span> {report.data.report.category}
            </p>
            <p>
            ...
            <p
              style={{
                color:
                  report.data.report.status === "pending"
                    ? "#d66a6a"
                    : report.data.report.status === "active"
                    ? "#e9b949"
                    : "#647acb",
            ...
            >
              <span>Status:</span> {report.data.report.status}
            </p>
          </div>
          <div>
            <p>
              <span>Description:</span> {report.data.report.description}
            </p>
            <span>
              {report.data.report.media.length > 0 && (
                <span>Images/Videos</span>
              )}
              <div>
                {report.data.report.media.length > 0 &&
                  report.data.report.media.map((img) => (
                    <img key={img} src={img} alt={report.data.report.title} />
                  ))}
              </div>
            </span>
          </div>
          {report.data.report.response && (
            <div>
              ...
              {report.data.report.response && (
                <>
                  <p>
                    <span>Updated at:</span> {updateDay}
                    {updateDay === 1
                      ? "st"
                      : updateDay === 2
                      ? "nd"
                      ...
                      ? "rd"
                      : "th"}
                    , {months[updateMonth]} {updateYear}, {updateCode}{" "}
                    {updatePeriod}
                  </p>
                  <p>
                    <span>Description:</span> {report.data.report.response}
                  </p>
                </>
              )}
            </div>
          )}
          {report.data.report.status !== "responded" &&
