...
export default function Reports() {
  ...
  useEffect(() => {
    ...
    instance.get(`reports?page=${page.currPage}&${query}`)
    .then(res => {
  ...
  return (
    <div className='reports'>
      <div className='reports-top'>
        <h2>Reports</h2>
        {userType !== 'officer' && <Link to='create-report'>
          Create Report
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M19 11h-6V5h-2v6H5v2h6v6h2v-6h6z"></path></svg>
        </Link>}
      </div>

      <form className={showFilter ? 'showFilter reports-middle' : 'hideFilter reports-middle'} onSubmit={prodFilter} >
        <select onChange={updateFilterData} name='category' id='category' value={filters.category}>
        ...
        <select onChange={updateFilterData} name='sort' id='sort' value={filters.sort}>
        ...
        <select onChange={updateFilterData} name='status' id='status' value={filters.status}>
        ...
        <input onChange={updateFilterData} value={filters.search} type='text' name='search' id='search' placeholder='Search based on title' />
        </div>

        <button type='submit'>Filter</button>
      </form>
      {!showFilter ? <svg onClick={() => setShowFilter(prev => !prev)} width='40' height='25' viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><defs></defs><title/><g data-name="Layer 2" id="Layer_2"><path class="cls-1" d="M16.47,16.88,26.34,7a1,1,0,0,0-1.41-1.41l-9.06,9.06-8.8-8.8a1,1,0,0,0-1.41,0h0a1,1,0,0,0,0,1.42l9.61,9.61A.85.85,0,0,0,16.47,16.88Z"/><path class="cls-1" d="M16.47,26.46l9.87-9.88a1,1,0,0,0-1.41-1.41l-9.06,9.06-8.8-8.8a1,1,0,0,0-1.41,0h0a1,1,0,0,0,0,1.41l9.61,9.62A.85.85,0,0,0,16.47,26.46Z"/></g></svg> : <svg onClick={() => setShowFilter(prev => !prev)} width='40' height='25' viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><defs></defs><title/><g data-name="Layer 2" id="Layer_2"><path className="cls-1" d="M16.47,15.12,26.34,25a1,1,0,0,1-1.41,1.41l-9.06-9.06-8.8,8.8a1,1,0,0,1-1.41,0h0a1,1,0,0,1,0-1.42l9.61-9.61A.85.85,0,0,1,16.47,15.12Z"/><path className="cls-1" d="M16.47,5.54l9.87,9.88a1,1,0,0,1-1.41,1.41L15.87,7.77l-8.8,8.8a1,1,0,0,1-1.41,0h0a1,1,0,0,1,0-1.41l9.61-9.62A.85.85,0,0,1,16.47,5.54Z"/></g></svg>}
    ...
    </table> : <p style={{padding: '1em', textAlign: 'center', fontWeight: '700', color: 'var(--darkBlue)'}}>No reports found</p> : <Loader />}
